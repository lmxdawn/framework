...
public class MemberController extends BaseAdminController {
    ...
    @RequiresPermissions("member:update-invite")
    @PostMapping(value = "update-invite")
    @AccessLog(module = AdminModule.MEMBER, operation = "会员层级迁移")
    public MessageResult updateInvite(@RequestParam(value = "memberId") Long memberId,
                                      @RequestParam(value = "toUsername") String toUsername) {
        if (memberId == null) {
            return error("id必须传参");
        }
        if (toUsername == null) {
            return error("请输入邮箱或者手机号");
        }

        Member toMember = null;
        if (toUsername.contains("@")) {
            toMember = memberService.findByEmail(toUsername);
        } else {
            toMember = memberService.findByPhone(toUsername);
        }

        if (toMember == null) {
            return error("当前邮箱或者手机号不存在");
        }
        ...
        one.setInviterId(toMember.getId());
        Member save = memberService.save(one);
        return success(save);
    }
