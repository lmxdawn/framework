...
import com.bizzan.bitrade.model.vo.OtcOrderExcelVO;
...
public class AdminOrderController extends BaseController {
    ...
    public MessageResult page(
        ...
        if (screen.getIsOut() != null && screen.getIsOut() == 1) {
            ...
            List<OtcOrderVO> otcOrderVOS = orderService.outExcelAll(predicate, pageModel);

            List<OtcOrderExcelVO> voList = new ArrayList<>();
            for (OtcOrderVO v : otcOrderVOS) {
                OtcOrderExcelVO vo = new OtcOrderExcelVO();
                BeanUtils.copyProperties(v, vo);

                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

                vo.setCreateTime(sdf.format(v.getCreateTime()));
                AdvertiseType advertiseType = v.getAdvertiseType();
                String advertiseTypeStr = "卖出";
                if (advertiseType == AdvertiseType.BUY) {
                    advertiseTypeStr = "买入";
                }
                vo.setAdvertiseType(advertiseTypeStr);
                vo.setNumber(v.getNumber().toPlainString());
                vo.setMoney(v.getMoney().toPlainString());
                vo.setFee(v.getFee().toPlainString());

                OrderStatus status = v.getStatus();
                String statusStr = "已取消";
                if (status == OrderStatus.APPEAL) {
                    statusStr = "申诉中";
                } else if (status == OrderStatus.NONPAYMENT) {
                    statusStr = "未付款";
            ...
            ExcelUtil.listToExcel(voList, OtcOrderExcelVO.class.getDeclaredFields(), response.getOutputStream());
