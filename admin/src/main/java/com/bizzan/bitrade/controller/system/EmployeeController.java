...
import com.bizzan.bitrade.util.GoogleAuthenticator;
...
public class EmployeeController extends BaseAdminController {
    ...
    public MessageResult adminLogin(@RequestParam(value = "username", required = true) String username,
                                    ...
                                    @RequestParam(value = "googleKey", required = true) String googleKey,
        ...
        boolean isUpdateGoogleKey = true;
        // 验证Google验证码
        if (admin.getGoogleKey() != null && StringUtils.isNotBlank(admin.getGoogleKey())) {
            googleKey = admin.getGoogleKey();
            isUpdateGoogleKey = false;
        }

        if (StringUtils.isBlank(googleKey) || StringUtils.isBlank(googleCode)) {
            return error("Google验证码错误");
        }

        Boolean aBoolean = GoogleAuthenticator.authCode(googleCode, googleKey);
        if (!aBoolean) {
            return error("Google验证码错误");
        }
        ...
        for (String s : adminList) {
            sendEmailMsg(s, "管理员(UserName: " + username + ", Phone: " + admin.getMobilePhone() + ") 登录后台", "管理员登录通知");
        }

        if (isUpdateGoogleKey) {
            // 修改数据库里的GoogleKey
    ...
    @RequestMapping(value = "/getGoogleCodeKey")
    @ResponseBody
    @AccessLog(module = AdminModule.SYSTEM, operation = "获取Google验证码")
    public MessageResult getGoogleCodeKey() {

        // 生成Google验证码
        String user = company;
        String secret = GoogleAuthenticator.genSecret(user);
        String qrBarcodeURL = GoogleAuthenticator.getQRBarcodeURL(user, secret);

        Map<String, String> map = new HashMap<>();
        map.put("googleKey", secret);
        map.put("googleKeyImgUrl", qrBarcodeURL);
...
//        }
    ...
    @RequiresPermissions("system:employee:update-googleKey")
    @PostMapping("update-googleKey")
    @ResponseBody
    public MessageResult updateGoogleKey(Long id, String password, String googleOldCode, String googleKey, String googleCode) {
        Assert.notNull(id, "admin id 不能为null");
        Assert.notNull(password, "请输入密码");
        Assert.notNull(googleOldCode, "请输入旧Google验证码");
        Assert.notNull(googleKey, "请获取新Google绑定码");
        Assert.notNull(googleCode, "请输入新Google验证码");
        Admin admin = adminService.findOne(id);
        password = Encrypt.MD5(password + md5Key);
        Assert.isTrue(password.equalsIgnoreCase(admin.getPassword()), "密码错误");

        // 验证旧Google码
        Boolean aBoolean = GoogleAuthenticator.authCode(googleOldCode, admin.getGoogleKey());
        if (!aBoolean) {
            return MessageResult.error("旧Google验证码错误");
        }

        Boolean aBoolean1 = GoogleAuthenticator.authCode(googleCode, googleKey);
        if (!aBoolean1) {
            return MessageResult.error("新Google验证码错误");
        }
