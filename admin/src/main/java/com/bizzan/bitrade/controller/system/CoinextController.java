...
public class CoinextController extends BaseAdminController {
    ...
    public MessageResult merge(@Valid Coinext coinext, BindingResult bindingResult) throws Exception {
        ...
        if (one == null || one.getMainaddress() == null || !one.getMainaddress().equals(coinext.getMainaddress())) {
            if (StringUtils.isNotBlank(coinext.getMainaddress())) {
                coinext.setMainaddress(DESEncryptUtil.EncryptData(coinext.getMainaddress()));
            }
        }
