...
import com.bizzan.bitrade.model.vo.MemberWalletVO;
...
public class MemberWalletController extends BaseAdminController {
    ...
    public MessageResult getBalance(
        ...
        if (screen.getIsOut() == 1) {
            List<MemberWalletDTO> list = memberWalletService.joinFindAll(criteria, qMember, qMemberWallet, pageModel);
            List<MemberWalletVO> collect = list.stream().map(v -> {
                MemberWalletVO vo = new MemberWalletVO();
                BeanUtils.copyProperties(v, vo);
                vo.setBalance(v.getBalance().toPlainString());
                vo.setFrozenBalance(v.getFrozenBalance().toPlainString());
                vo.setAllBalance(v.getAllBalance().toPlainString());
                return vo;
            }).collect(Collectors.toList());
            // 导出
            ExcelUtil.listToExcel(collect, MemberWalletVO.class.getDeclaredFields(),response.getOutputStream());
            return null;
        }
    ...
    public MessageResult recharge(
        ...
        Boolean aBoolean = GoogleAuthenticator.authCode(googleCode, admin.getGoogleKey());
        if (!aBoolean) {
            return error("Google验证码错误");
        }
