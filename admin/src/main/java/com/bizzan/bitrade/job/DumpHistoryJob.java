...
public class DumpHistoryJob {
    ...
    /**
     *  每天1点开始，9点结束，每次删除1500条
     *  注意，删除订单主表和详情表时，只删除机器人数据（详见数据层的sql实现）
     * */
	@Scheduled(cron = "0 */1 0-9 * * *")
	public void deleteHistoryOrders(){
		log.info("开始清理交易历史数据");
		long beforeTime = System.currentTimeMillis() - (24 * 60 * 60 * 1000); // 1天前
		log.info("清除指定时间之前的订单："+beforeTime);
		List<Object> list = exchangeOrderService.queryHistoryDelete(beforeTime);
		log.info("历史订单数量："+list.size());
		int deleteCount = 0;
		if(list != null && list.size() > 0) {
			// 清除总表
			deleteCount = exchangeOrderService.deleteHistory(beforeTime);
			Query query=new Query(Criteria.where("orderId").in(list));
		...
		now.setTime(today);  
		now.set(Calendar.DATE,now.get(Calendar.DATE) - 2); // 清理2天前数据
