...
public class KLineSyncJob {
    ...
    public void getKLine(String symbol, String period) {
        ...
        map.put("size", "1");
        map.put("symbol", symbol.replace("/", "").toLowerCase());

        String url = "https://api.huobi.pro/market/history/kline";
        String s = OkHttpUtil.get(url, map);

        HuobiKLineVo huobiKLineVo = JSONObject.parseObject(s, HuobiKLineVo.class);

        String status = huobiKLineVo.getStatus();
        if (!status.equals("ok")) {
            return;
        }
        List<HuobiKLineVo.DataDTO> data = huobiKLineVo.getData();
        HuobiKLineVo.DataDTO dataDTO = data.get(0);

        KLine kline = new KLine(period);
        kline.setClosePrice(BigDecimal.valueOf(dataDTO.getClose()));
        kline.setCount(dataDTO.getCount());
        kline.setHighestPrice(BigDecimal.valueOf(dataDTO.getHigh()));
        kline.setLowestPrice(BigDecimal.valueOf(dataDTO.getLow()));
        kline.setOpenPrice(BigDecimal.valueOf(dataDTO.getOpen()));
        kline.setTime(dataDTO.getId());
        // 火币成交量的百分比
        double VOLUME_PERCENT = 0.13;
        kline.setTurnover(BigDecimal.valueOf(dataDTO.getAmount()).multiply(BigDecimal.valueOf(VOLUME_PERCENT)));
        kline.setVolume(BigDecimal.valueOf(dataDTO.getVol()).multiply(BigDecimal.valueOf(VOLUME_PERCENT)));
        ...
        for(CoinSyncItem coinItem : coinList) {
            for(String period : PERIOD) {
                ...
                if(period.equals("1min") && timeGap >= 60) { // 超出1分钟
                    ...
                    // 获取当前时间段的k线
                    getKLine(coinItem.symbol, period);
                }
