...
public class ContractCoinMatch {
    ...
    public void dealOpenOrder(ContractOrderEntrust order){
        ...
        // 增加资产变更记录
        String symbol = contractCoin.getSymbol().split("/")[1];
        ...
        // 查找上级
        Member one = memberService.findOne(order.getMemberId());
        if (one != null && one.getInviterId() != null) {
            // 查找推广配置
            ...
            if (rewardPromotionSetting != null && !(DateUtil.diffDays(new Date(), one.getRegistrationTime()) > rewardPromotionSetting.getEffectiveTime())) {
                Member member1 = memberService.findOne(one.getInviterId());
                JSONObject jsonObject = JSONObject.parseObject(rewardPromotionSetting.getInfo());
                BigDecimal reward = BigDecimalUtils.mulRound(order.getOpenFee(), BigDecimalUtils.getRate(jsonObject.getBigDecimal("one")), 8);
                MemberWallet memberWallet = memberWalletService.findByCoinUnitAndMemberId(symbol, member1.getId());
                memberWalletService.increaseBalance(memberWallet.getId(), reward);
                MemberTransaction memberTransaction1 = new MemberTransaction();
                memberTransaction1.setAmount(reward);
                memberTransaction1.setFee(BigDecimal.ZERO);
                memberTransaction1.setMemberId(member1.getId());
                memberTransaction1.setSymbol(symbol);
                memberTransaction1.setType(TransactionType.PROMOTION_AWARD);
                memberTransaction1.setDiscountFee("0");
                memberTransaction1.setRealFee("0");
                memberTransaction1.setUsdtPrice(BigDecimal.valueOf(1));
    ...
    public void dealCloseOrder(ContractOrderEntrust order) {
        ...
        if (order.getDirection() == ContractOrderDirection.BUY) { // 买入平空
            ...
            MemberTransaction memberTransaction = new MemberTransaction();
            memberTransaction.setFee(BigDecimal.ZERO);
            memberTransaction.setAmount(BigDecimal.ZERO.subtract(closeFee));
            memberTransaction.setMemberId(order.getMemberId());
            memberTransaction.setSymbol(symbol);
            memberTransaction.setType(TransactionType.CONTRACT_FEE);
            memberTransaction.setCreateTime(DateUtil.getCurrentDate());
            ...
            if (one != null && one.getInviterId() != null) {
                ...
                if (rewardPromotionSetting != null && !(DateUtil.diffDays(new Date(), one.getRegistrationTime()) > rewardPromotionSetting.getEffectiveTime())) {
                    Member member1 = memberService.findOne(one.getInviterId());
                    JSONObject jsonObject = JSONObject.parseObject(rewardPromotionSetting.getInfo());
                    BigDecimal reward = BigDecimalUtils.mulRound(closeFee, BigDecimalUtils.getRate(jsonObject.getBigDecimal("one")), 8);
                    MemberWallet memberWallet = memberWalletService.findByCoinUnitAndMemberId(symbol, member1.getId());
                    memberWalletService.increaseBalance(memberWallet.getId(), reward);
                    MemberTransaction memberTransaction1 = new MemberTransaction();
                    memberTransaction1.setAmount(reward);
...
//        logger.info("{} 盘面更新：大小-{}", symbol, tradeArrayList.size());
    ...
    public void setRewardPromotionSettingService(RewardPromotionSettingService rewardPromotionSettingService) {
        this.rewardPromotionSettingService = rewardPromotionSettingService;
    }

    public void setRewardRecordService(RewardRecordService rewardRecordService) {
