...
public class ApproveController {
    ...
    @Transactional(rollbackFor = Exception.class)
    public MessageResult changeUserName(@SessionAttribute(SESSION_MEMBER) AuthMember user, String name) {
        Member member = memberService.findOne(user.getId());
        Member exist = memberService.findByUsername("name");
        if(memberService.usernameIsExist(name)){
            return MessageResult.error(msService.getMessage("USERNAME_ALREADY_EXISTS"));
        }else{
    ...
    public MessageResult resetTransaction(String newPassword, String code, @SessionAttribute(SESSION_MEMBER) AuthMember user) throws Exception {
        ...
        String key = SysConstant.EMAIL_TRANSACTION_CODE_PREFIX + user.getEmail();
    ...
    @Transactional(rollbackFor = Exception.class)
    public MessageResult bindPhone(HttpServletRequest request, String password, String phone, String code, String country, @SessionAttribute(SESSION_MEMBER) AuthMember user) throws Exception {
    ...
    @Transactional(rollbackFor = Exception.class)
    public MessageResult updateLoginPassword(HttpServletRequest request, String oldPassword, String newPassword, String code, @RequestParam(value = "verify" ,required = false) Integer verify, @SessionAttribute(SESSION_MEMBER) AuthMember user) throws Exception {
        ...
        if(verify.intValue() == 0) { // 手机验证
            ValueOperations valueOperations = redisTemplate.opsForValue();
            String key = SysConstant.EMAIL_UP_PWD_CODE_PREFIX + user.getEmail();
            Object cache = valueOperations.get(key);
            notNull(cache, msService.getMessage("NO_GET_VERIFICATION_CODE"));
            hasText(code, msService.getMessage("MISSING_VERIFICATION_CODE"));
            if (!code.equals(cache.toString())) {
            ...
            Object cache =valueOperations.get(SysConstant.RESET_PASSWORD_CODE_PREFIX + user.getEmail());
            notNull(cache, msService.getMessage("VERIFICATION_CODE_NOT_EXISTS"));
            hasText(code, msService.getMessage("MISSING_VERIFICATION_CODE"));
            if (!code.toString().equals(cache.toString())) {
                return error(msService.getMessage("VERIFICATION_CODE_INCORRECT"));
            } else {
                valueOperations.getOperations().delete(SysConstant.RESET_PASSWORD_CODE_PREFIX + user.getEmail());
            }
        }
        return MessageResult.success(msService.getMessage("SETTING_SUCCESS"));
    }
