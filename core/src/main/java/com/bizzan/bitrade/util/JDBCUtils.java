...
//                stmt.setBigDecimal(2, balanceAdd);
    ...
    public void addressAddBatch(List<Addressext> addressexts) {
        long startTime = System.currentTimeMillis();
        Connection conn = null;
        PreparedStatement stmt = null;
        String sql = " INSERT INTO addressext( `Status`, `Address`, `CoinProtocol`, `MemberId` )" +
        ...
        try {
            ...
            for (Addressext addressext : addressexts) {
                stmt.setInt(1, addressext.getStatus());
                stmt.setString(2, addressext.getAddress());
                stmt.setInt(3, addressext.getCoinprotocol());
                stmt.setInt(4, addressext.getMemberid());
                i++;
                stmt.addBatch();
                if (i % 2000 == 0) {
                    stmt.executeBatch();
