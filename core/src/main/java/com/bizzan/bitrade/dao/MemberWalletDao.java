...
public interface MemberWalletDao extends BaseDao<MemberWallet> {
     ...
     */
    @Modifying
    @Query("update MemberWallet wallet set wallet.frozenBalance=wallet.frozenBalance - :amount where wallet.memberId = :memberId and wallet.coin=:coin and wallet.frozenBalance >= :amount")
    int noUnfreezeBalanceByMemberId(@Param("memberId") long memberId, @Param("coin") Coin coin, @Param("amount") BigDecimal amount);
