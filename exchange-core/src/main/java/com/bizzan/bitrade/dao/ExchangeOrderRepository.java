...
public interface ExchangeOrderRepository extends JpaRepository<ExchangeOrder, String>, JpaSpecificationExecutor<ExchangeOrder>, QueryDslPredicateExecutor<ExchangeOrder> {
    ...
    @Modifying
    @Query(value = "update exchange_order set fee = fee + ?1  where order_id = ?2", nativeQuery = true)
    int increaseFee(BigDecimal fee, String orderId);
    ...
    @Query(value = "select order_id from exchange_order where member_id=1 and `time` < :beforeTime and status<>0 limit 3000", nativeQuery = true)
    List<Object> queryHistoryDeleteList(@Param("beforeTime") long beforeTime);
    
    @Transactional
    @Modifying
    @Query(value = "delete from exchange_order where member_id = 1 and status <> 0 and `time` < :beforeTime limit 3000", nativeQuery = true)
