...
public interface ContractOrderEntrustRepository extends JpaRepository<ContractOrderEntrust, Long>, JpaSpecificationExecutor<ContractOrderEntrust>, QueryDslPredicateExecutor<ContractOrderEntrust> {
    ...
    @Query("select SUM(a.profitAndLoss) from ContractOrderEntrust a where a.baseSymbol = :baseSymbol and a.coinSymbol = :coinSymbol")
    BigDecimal sumProfitAndLossBuySymbol(@Param("baseSymbol") String baseSymbol, @Param("coinSymbol") String coinSymbol);
}
