...
public class CoinTrader {
    ...
    public ExchangeOrder cancelOrder(ExchangeOrder exchangeOrder){
            ...
            synchronized (list) {
                ...
                while ((orderIterator.hasNext())) {
                    ...
                    if (order.getOrderId().equalsIgnoreCase(exchangeOrder.getOrderId())) {
                        orderIterator.remove();
                        logger.info("撤销订单：orderId={}", order.getOrderId());
                        onRemoveOrder(order);
                        return order;
                    }
                }
        ...
        logger.info("撤销订单222：orderId={}", exchangeOrder.getOrderId());
        // 找不到订单，也要删除
        TradePlate list2 = null;
        if (exchangeOrder.getDirection() == ExchangeOrderDirection.BUY) {
            list2 = buyTradePlate;
        } else {
            list2 = sellTradePlate;
            sendTradePlateMessage(sellTradePlate);
        }
        synchronized (list2) {
            buyTradePlate.remove(exchangeOrder);
            sendTradePlateMessage(list2);
        }
        logger.info("撤销订单3333：orderId={}", exchangeOrder.getOrderId());
